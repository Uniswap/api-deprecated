...
export const TOP_PAIRS = gql`
  ...
  query(
    ...
    pairs: pairDayDatas(
      ...
      orderBy: reserveUSD
      orderDirection: desc
      where: { token0_not_in: $excludeTokenIds, token1_not_in: $excludeTokenIds, date: $todayStartTime }
    ) {
      id
      token0 {
        ...TokenInfo
    ...
    yesterdaysVolume: pairDayDatas(
      ...
      orderBy: reserveUSD
      orderDirection: desc
      where: { token0_not_in: $excludeTokenIds, token1_not_in: $excludeTokenIds, date: $yesterdayStartTime }
    ) @include(if: $detailed) {
      id
      dailyVolumeToken0
      dailyVolumeToken1
